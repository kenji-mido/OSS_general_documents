...
If you think that you have identified a security vulnerability in AITRIOS Edge Device Sensor, please send us your report with as much context as possible via [our contact form](https://support.aitrios.sony-semicon.com/hc/en-us/requests/new).
...
- Keep API compatibility in mind when you change code in core functionality. Any non-backward-compatible public API changes, i.e. breaking changes to header files in the `aitrios-edge-device-sensor/src/` folder, will require a major version increment.
...
Bear in mind that when you contribute a new feature, the maintenance burden is transferred to the aitrios-edge-device-sensor team. This means that the benefit of the contribution must be compared against the cost of maintaining the feature.
